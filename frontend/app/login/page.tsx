  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 pt-20">
      <div className="card-minimal w-full max-w-md rounded-2xl p-8">
        ...
        <p className="mb-8 text-sm text-zinc-500 dark:text-zinc-400">Sign in to your account to continue.</p>

        {/* TABS FOR SELECTING LOGIN METHOD */}
        {(!magicLinkSent && !otpSent) && (
          <div className="mb-8 flex gap-1 rounded-xl bg-zinc-100 dark:bg-zinc-900 p-1">
            <button 
              onClick={() => { setMethod('password'); setGlobalError(null); }}
              className={`flex-1 rounded-lg py-2 text-xs font-bold uppercase tracking-wider transition-all ${method === 'password' ? 'bg-white dark:bg-zinc-800 text-black dark:text-white shadow-sm' : 'text-zinc-500 hover:text-black dark:text-zinc-400 dark:hover:text-white'}`}
            >
              Password
            </button>
            <button 
              onClick={() => { setMethod('magic_link'); setGlobalError(null); }}
              className={`flex-1 rounded-lg py-2 text-xs font-bold uppercase tracking-wider transition-all ${method === 'magic_link' ? 'bg-white dark:bg-zinc-800 text-black dark:text-white shadow-sm' : 'text-zinc-500 hover:text-black dark:text-zinc-400 dark:hover:text-white'}`}
            >
              Magic Link
            </button>
            <button 
              onClick={() => { setMethod('otp'); setGlobalError(null); }}
              className={`flex-1 rounded-lg py-2 text-xs font-bold uppercase tracking-wider transition-all ${method === 'otp' ? 'bg-white dark:bg-zinc-800 text-black dark:text-white shadow-sm' : 'text-zinc-500 hover:text-black dark:text-zinc-400 dark:hover:text-white'}`}
            >
              OTP
            </button>
          </div>
          ...
          <div>
            {!magicLinkSent ? (
              ...
              <div className="py-4 text-center">
                ...
                <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">We've sent a sign-in link to {emailForm.getValues('email')}.</p>
                <button onClick={() => setMagicLinkSent(false)} className="mt-6 text-xs font-bold text-zinc-400 hover:text-black dark:hover:text-white">Try another email</button>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            {!otpSent ? (
              ...
              <form onSubmit={otpForm.handleSubmit(onOtpVerify)} className="space-y-5">
                <p className="text-center text-sm text-zinc-500 dark:text-zinc-400">Enter the 6-digit code sent to <br/> <strong className="text-black dark:text-white">{otpEmail}</strong></p>
                ...
                <div className="space-y-1">
                  <input 
                    ...
                    type="text"
                    maxLength={6}
                    className="input-minimal w-full text-center text-2xl font-bold tracking-[0.5em] rounded-lg px-4 py-4 text-black dark:text-white dark:bg-zinc-950 dark:border-zinc-800"
                    placeholder="000000"
                  />
                ...
                <button type="button" onClick={() => setOtpSent(false)} className="w-full text-center text-xs font-bold text-zinc-400 hover:text-black dark:hover:text-white">Use a different email</button>
              </form>
            )}
          </div>
        )}
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button 
                ...
                className="flex items-center justify-center gap-3 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-black py-3 transition-all hover:bg-zinc-50 dark:hover:bg-zinc-900"
              >
                ...
                <span className="text-sm font-semibold text-black dark:text-white">Google</span>
              </button>
              ...
              >
                ...
                <span className="text-sm font-semibold text-black dark:text-white">GitHub</span>
              </button>
            </div>
          </>
        )}
